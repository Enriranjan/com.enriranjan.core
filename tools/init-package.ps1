...
if ($PackageId -notmatch '^[a-z][a-z0-9-]*$') {
    Write-Error "package-id must be lowercase letters/digits/hyphens, starting with a letter (got '$PackageId')."
}
if ($PackageName -notmatch '^[A-Z][A-Za-z0-9]*$') {
    Write-Error "PackageName must be PascalCase, starting with an uppercase letter (got '$PackageName')."
}
if ([string]::IsNullOrWhiteSpace($DisplayName)) {
